# Veepag Tech Test — Architectural Plan (V2)

**Strategy**: Nail Core MVP first, impress with improvements later. 8-hour window.

**Changes from V1:**
- Products: replaced two separate indexes with a single compound index `{ status: 1, createdAt: -1 }`
- Products: added `currency` field (default `BRL`) — carries into `productSnapshot`
- Subscriptions: removed redundant standalone `{ customerCpf: 1 }` index (already covered by compound left-prefix)
...
- **currency field**: hardcoded `BRL` for now; makes future multi-currency support a non-breaking addition
- **Soft-delete via status**: products are never deleted, preserving historical integrity

**Indexes:**
- `{ status: 1, createdAt: -1 }` — compound: filter by status + sort by date in a single index scan (replaces two separate indexes; left-prefix rule means queries on `status` alone still use this index)
...
| `productSnapshot` | Object | `{ name, price, currency, billingCycle }` — captured at creation |
...
**Design decisions:**
- **productSnapshot**: denormalizes name, price, currency, billingCycle at subscription time. Prevents broken data if product price changes or product is deactivated. This is how Stripe subscriptions work.
...
- `{ customerCpf: 1, productId: 1, status: 1 }` — compound: duplicate-active check AND covers all `customerCpf`-only queries via left-prefix rule (replaces the separate `{ customerCpf: 1 }` index)

> **Left-prefix note**: `{ customerCpf: 1, productId: 1, status: 1 }` can serve queries that filter only by `customerCpf` (e.g., the "My Subscriptions" page), so a standalone `{ customerCpf: 1 }` index would be redundant.
...
| `currency` | String | required, matches productSnapshot.currency |
...
- **amount + currency copied from productSnapshot** — transaction is a financial record; immutable snapshot of what was charged
...
│       │   ├── checkout.reducer.ts      # useReducer state: idle|loading|product_loaded|submitting|success|error
│       │   ├── checkout.types.ts
│       │   └── components/
...
```
POST   /products
  Body: { name, description?, price, currency?, billingCycle }
...
- [ ] Define `product.schema.ts` (all fields + compound index via `@index` decorator)
- [ ] Define `subscription.schema.ts` (productSnapshot subdoc including currency, all enums)
- [ ] Define `transaction.schema.ts` (paymentDetails subdoc, currency field)
...
- [ ] `ProductCard.tsx`: displays name, description, price (formatted from cents + currency), billing cycle
...
- [ ] `TransactionHistory.tsx`: collapsible per subscription; shows amount, currency, status, date
...
- [ ] `README.md`:
  ...
  - Technical decisions (price in cents, currency field, productSnapshot, CPF validation, repository pattern, compound indexes)
  - Business rules implemented
  - Payment simulation rule (exact card numbers for approved/declined)
  - Future improvements (MongoDB transactions for atomicity, renewal billing, customer auth, multi-currency)
